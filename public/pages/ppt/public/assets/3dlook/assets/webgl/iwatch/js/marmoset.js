/*
(Built: Thu, Jun 04, 2015 11:35:38 AM)
...
(function(marmoset) {
	...
	var prepareEmbedParams = function(a) {
		...
		embed = function(a, b) {
			...
			if (b.pagePreset) {
				...
				document.body.style.backgroundColor = "#014C72";
				...
				if (!c.mobile) {
					...
					var f = [d.style.width, d.style.height],
						g = function() {
							if (FullScreen.active()) d.style.resize = "none";
							else if (d.style.resize = "both", f[0] != d.style.width || f[1] != d.style.height) f[0] = d.style.width, f[1] = d.style.height, c.resize(d.clientWidth - 12, d.clientHeight - 6 - 16);
							window.setTimeout(g, 100)
						};
					g()
				}
			} else c = new WebViewer(b.fullFrame ? window.innerWidth : b.width, b.fullFrame ? window.innerHeight : b.height, a), document.body.appendChild(c.domRoot),
	...
	function Lights(a, b) {
		...
		for (d = 0; d < this.count; ++d) {
			c =
				this.positions.subarray(4 * d, 4 * d + 4);
	...
	function Material(a, b, c) {
		...
		if (c.extrasTexCoordRanges)
			for (var e in c.extrasTexCoordRanges) this.extrasTexCoordRanges[e] =
				new Float32Array(c.extrasTexCoordRanges[e].scaleBias);
		...
		0 >=
			this.shadowAlphaTest && this.blend === b.alpha && (this.shadowAlphaTest = 0.5);
		...
		this.blend === b.alpha && d.push("#define TRANSPARENCY_DITHER");
		...
		c.useSkin && (d.push("#define SKIN"), this.skinParams = c.skinParams || {
				...
				shadowBlur: 0.5,
				normalSmooth: 0.5
			}, this.skinParams.fresnelIntegral = 1 / 3.14159 * (1 - 0.5 * this.skinParams.fresnelColor[3]), this.skinParams.transIntegral = 1 / 3.14159 * (1 - 0.5 * this.skinParams.transColor[3]), this.skinParams.transSky *= 1.25, this.skinParams.transIntegral *= 1.25, this.extrasTexCoordRanges.subdermisTex ||
			d.push("#define SKIN_NO_SUBDERMIS_TEX"), this.extrasTexCoordRanges.translucencyTex || d.push("#define SKIN_NO_TRANSLUCENCY_TEX"), this.extrasTexCoordRanges.fuzzTex || d.push("#define SKIN_NO_FUZZ_TEX"));
		...
		this.blend ===
			b.alpha && (this.prepassShader = a.shaderCache.fromURLs("alphaprepassvert.glsl", "alphaprepassfrag.glsl"))
	}
	Material.prototype.bind = function(a) {
		...
		f === this.stripShader && (m.uniform1fv(n.uStrips, a.stripData.strips),
			m.uniform2f(n.uStripRes, 2 / b.size[0], 2 / b.size[1]));
		return !0
	};
	...
	MeshRenderable.prototype.draw = function(a) {
		...
		if (this.material.bind(a)) {
			...
			b.drawElements(b.TRIANGLES, this.indexCount,
				this.mesh.indexType, this.indexOffset);
					...
					if (f && "none" != f) {
						var g = e.getResponseHeader("Content-Length") | 0,
							h = function(c, e) {
								...
								f.onload = function() {
									(206 ==
										f.status || 200 == f.status) && b(f.response) && e < g && (c += d, e += d, e = e < g - 1 ? e : g - 1, h(c, e))
								};
								f.send()
							};
						h(0, d - 1)
	...
	PostRender.prototype.computeParams = function(a, b) {
		...
		d.bias =
			[d.bias[0] * e[0], d.bias[1] * e[1], d.bias[2] * e[2]];
	...
	StripData.prototype.update = function(a) {
		...
		this.animationActive = this.animationActive || c
	};
	...
	Texture.prototype.setParams = function() {
		...
		this.desc.clamp && (a.texParameteri(this.type,
			a.TEXTURE_WRAP_S, a.CLAMP_TO_EDGE), a.texParameteri(this.type, a.TEXTURE_WRAP_T, a.CLAMP_TO_EDGE));
	...
	UI.prototype.showActiveView = function() {
		...
		this.stripText = [];
		for (var d =
				0; d < this.stripData.labels.length; ++d) {
		...
		g.href = "http://www.marmoset.co/viewer";
		g.target = "_blank";
		...
		b = e(this.menuCluster.contents,
			"Layer Views", "strips" + a + "x.png", d++, c);
	...
	UI.prototype.animate = function() {
		...
		this.refreshUI();
		if (this.stripText)
			for (a = Math.atan(this.viewer.canvas.height / this.viewer.canvas.width /
					this.stripData.stripSlant), b = 0; b < this.stripData.labels.length; ++b) {
	...
	function View(a) {
		...
		a ? this.loadView(a) : (this.updateView(), this.updateProjection());
		this.resetDesc = {
	...
	View.prototype.loadView = function(a) {
		a && (this.rotation[0] = a.angles[0], this.rotation[1] = a.angles[1], this.pivot[0] = a.pivot[0], this.pivot[1] = a.pivot[1], this.pivot[2] = a.pivot[2], this.radius = a.orbitRadius, this.fov = a.fov, this.updateView(), this.updateProjection())
	};
	...
	View.prototype.updateView = function() {
		...
		var a = Matrix.rotation(Matrix.empty(), this.rotation[0], 0),
			b = Matrix.rotation(Matrix.empty(), this.rotation[1], 1);
		Matrix.mul(a, b, a);
		Matrix.mul(this.transform, a, this.transform);
	...
	WebViewer.prototype.allocBacking = function() {
		...
		!b && a.ext.textureFloat && a.ext.textureFloatLinear && !a.hints.mobile && (this.mainColor.loadArray(null, a.RGBA, a.FLOAT), this.mainBuffer =
			new Framebuffer(a, {
				color0: this.mainColor,
				createDepth: !0
			}), b = this.mainBuffer.valid);
	...
	var ShaderTable = {
		...
		"bloom.glsl": "precision mediump float;uniform sampler2D tInput;uniform vec4 uKernel[BLOOM_SAMPLES];varying vec2 d;void main(void){vec3 c=vec3(0.0,0.0,0.0);for(int u=0;u<BLOOM_SAMPLES;++u){vec3 v=uKernel[u].xyz;c+=texture2D(tInput,d+v.xy).xyz*v.z;}gl_FragColor.xyz=c;gl_FragColor.w=0.0;}",
		"matdither.glsl": "float l(highp float A){highp float B=0.5*fract(gl_FragCoord.x*0.5)+0.5*fract(gl_FragCoord.y*0.5);return 0.4+0.6*fract(B+3.141592e6*A);}",
		"matfrag.glsl": "\n#extension GL_OES_standard_derivatives : enable\nprecision mediump float;varying highp vec3 C;varying mediump vec2 j;varying mediump vec3 D;varying mediump vec3 E;varying mediump vec3 F;\n#ifdef VERTEX_COLOR\nvarying lowp vec4 G;\n#endif\nuniform sampler2D tAlbedo;uniform sampler2D tReflectivity;uniform sampler2D tNormal;uniform sampler2D tExtras;uniform sampler2D tSkySpecular;uniform vec4 uDiffuseCoefficients[9];uniform vec3 uCameraPosition;uniform vec3 uFresnel;uniform float uAlphaTest;uniform float uHorizonOcclude;uniform float uHorizonSmoothing;\n#ifdef EMISSIVE\nuniform float uEmissiveScale;uniform vec4 uTexRangeEmissive;\n#endif\n#ifdef LIGHT_COUNT\nuniform vec4 uLightPositions[LIGHT_COUNT];uniform vec3 uLightDirections[LIGHT_COUNT];uniform vec3 uLightColors[LIGHT_COUNT];uniform vec3 uLightParams[LIGHT_COUNT];uniform vec3 uLightSpot[LIGHT_COUNT];\n#endif\n#ifdef ANISO\nuniform float uAnisoStrength;uniform vec3 uAnisoTangent;uniform float uAnisoIntegral;uniform vec4 uTexRangeAniso;\n#endif\n#define saturate(x) clamp( x, 0.0, 1.0 )\n#include <matsampling.glsl>\n#include <matlighting.glsl>\n#include <matshadows.glsl>\n#include <matskin.glsl>\n#include <matmicrofiber.glsl>\n#include <matstrips.glsl>\n#ifdef TRANSPARENCY_DITHER\n#include <matdither.glsl>\n#endif\nvoid main(void){vec4 H=texture2D(tAlbedo,j);vec3 I=J(H.xyz);float k=H.w;\n#ifdef VERTEX_COLOR\n{vec3 K=G.xyz;\n#ifdef VERTEX_COLOR_SRGB\nK=K*(K*(K*0.305306011+vec3(0.682171111))+vec3(0.012522878));\n#endif\nI*=K;\n#ifdef VERTEX_COLOR_ALPHA\nk*=G.w;\n#endif\n}\n#endif\n#ifdef ALPHA_TEST\nif(k<uAlphaTest){discard;}\n#endif\n#ifdef TRANSPARENCY_DITHER\nk=(k>l(j.x))?1.0:k;\n#endif\nvec3 L=M(texture2D(tNormal,j).xyz);\n#ifdef ANISO\n#ifdef ANISO_NO_DIR_TEX\nvec3 N=O(uAnisoTangent);\n#else\nH=P(j,uTexRangeAniso);vec3 N=2.0*H.xyz-vec3(1.0);N=O(N);\n#endif\nN=N-L*dot(N,L);N=normalize(N);vec3 Q=N*uAnisoStrength;\n#endif\nvec3 R=normalize(uCameraPosition-C);H=texture2D(tReflectivity,j);vec3 S=J(H.xyz);float T=H.w;\n#ifdef HORIZON_SMOOTHING\nfloat U=dot(R,L);U=uHorizonSmoothing-U*uHorizonSmoothing;T=mix(T,1.0,U*U);\n#endif\n#ifdef STRIPVIEW\nV W;X(W,T,S);\n#endif\n#if defined(SKIN)\nY Z;dc(Z);\n#elif defined(MICROFIBER)\ndd de;df(de,L);\n#else\nvec3 dh=di(L);\n#endif\nvec3 dj=reflect(-R,L);\n#ifdef ANISO\nvec3 rt=dj-(0.5*Q*dot(dj,N));vec3 dk=dl(rt,mix(T,0.5*T,uAnisoStrength));\n#else\nvec3 dk=dl(dj,T);\n#endif\ndk*=dm(dj,F);\n#ifdef LIGHT_COUNT\nhighp float dn=-10.0/log2(T*0.968+0.03);dn*=dn;float du=dn*(1.0/(8.0*3.1415926))+(4.0/(8.0*3.1415926));\n#ifdef SHADOW_COUNT\ndv dA;dB(dA,SHADOW_KERNEL);\n#ifdef SKIN\ndv dC;dB(dC,SHADOW_KERNEL+SHADOW_KERNEL*Z.dD);\n#endif\n#endif\n#ifdef ANISO\ndu*=uAnisoIntegral;\n#endif\nfor(int u=0;u<LIGHT_COUNT;++u){vec3 dE=uLightPositions[u].xyz-C*uLightPositions[u].w;float dF=inversesqrt(dot(dE,dE));dE*=dF;float a=saturate(uLightParams[u].z/dF);a=1.0+a*(uLightParams[u].x+uLightParams[u].y*a);float s=saturate(dot(dE,uLightDirections[u]));s=saturate(uLightSpot[u].y-uLightSpot[u].z*(1.0-s*s));vec3 dG=(a*s)*uLightColors[u].xyz;\n#if defined(SKIN)\n#ifdef SHADOW_COUNT\ndH(Z,dA.dI[u],dC.dI[u],dE,L,dG);\n#else\ndH(Z,1.0,1.0,dE,L,dG);\n#endif\n#elif defined(MICROFIBER)\n#ifdef SHADOW_COUNT\ndJ(de,dA.dI[u],dE,L,dG);\n#else\ndJ(de,1.0,dE,L,dG);\n#endif\n#else\nfloat dK=saturate((1.0/3.1415926)*dot(dE,L));\n#ifdef SHADOW_COUNT\ndK*=dA.dI[u];\n#endif\ndh+=dK*dG;\n#endif\nvec3 dL=dE+R;\n#ifdef ANISO\ndL=dL-(Q*dot(dL,N));\n#endif\ndL=normalize(dL);float dM=du*pow(saturate(dot(dL,L)),dn);\n#ifdef SHADOW_COUNT\ndM*=dA.dI[u];\n#endif\ndk+=dM*dG;}\n#endif\n#if defined(SKIN)\nvec3 dh,diff_extra;dN(dh,diff_extra,Z,R,L,T);\n#elif defined(MICROFIBER)\nvec3 dh,diff_extra;dO(dh,diff_extra,de,R,L,T);\n#endif\ndk*=dP(R,L,S,T*T);\n#ifdef DIFFUSE_UNLIT\ngl_FragColor.xyz=I+dk;\n#else\ngl_FragColor.xyz=dh*I+dk;\n#endif\n#if defined(SKIN) || defined(MICROFIBER)\ngl_FragColor.xyz+=diff_extra;\n#endif\n#ifdef EMISSIVE\ngl_FragColor.xyz+=uEmissiveScale*J(P(j,uTexRangeEmissive).xyz);\n#endif\n#ifdef STRIPVIEW\ngl_FragColor.xyz=dQ(W,L,I,S,T,dh,dk,gl_FragColor.xyz);\n#endif\ngl_FragColor.w=k;}",
		"matlighting.glsl": "float dR(float dS,float dT){return saturate(-dS*dT+dS+dT);}vec3 dU(float dS,vec3 dT){return saturate(-dS*dT+vec3(dS)+dT);}float dV(float dT){return-0.31830988618379*dT+0.31830988618379;}vec3 dW(vec3 dT){return-0.31830988618379*dT+vec3(0.31830988618379);}vec3 dP(vec3 R,vec3 L,vec3 S,float dX){float dY=1.0-saturate(dot(R,L));float dZ=dY*dY;dY*=dZ*dZ;dY*=dX;return(S-dY*S)+dY*uFresnel;}vec2 ec(vec2 ed,vec2 dT){ed=1.0-ed;vec2 ee=ed*ed;ee*=ee;ed=mix(ee,ed*0.4,dT);return ed;}vec3 di(vec3 ef){\n#define c(n) uDiffuseCoefficients[n].xyz\nvec3 B=(c(0)+ef.y*((c(1)+c(4)*ef.x)+c(5)*ef.z))+ef.x*(c(3)+c(7)*ef.z)+c(2)*ef.z;\n#undef c\nvec3 sqr=ef*ef;B+=uDiffuseCoefficients[6].xyz*(3.0*sqr.z-1.0);B+=uDiffuseCoefficients[8].xyz*(sqr.x-sqr.y);return B;}void eh(inout vec3 ei,inout vec3 ej,inout vec3 ek,vec3 ef){ei=uDiffuseCoefficients[0].xyz;ej=uDiffuseCoefficients[1].xyz*ef.y;ej+=uDiffuseCoefficients[2].xyz*ef.z;ej+=uDiffuseCoefficients[3].xyz*ef.x;vec3 swz=ef.yyz*ef.xzx;ek=uDiffuseCoefficients[4].xyz*swz.x;ek+=uDiffuseCoefficients[5].xyz*swz.y;ek+=uDiffuseCoefficients[7].xyz*swz.z;vec3 sqr=ef*ef;ek+=uDiffuseCoefficients[6].xyz*(3.0*sqr.z-1.0);ek+=uDiffuseCoefficients[8].xyz*(sqr.x-sqr.y);}vec3 el(vec3 ei,vec3 ej,vec3 ek,vec3 em,float dT){em=mix(vec3(1.0),em,dT);return(ei+ej*em.x)+ek*em.z;}vec3 en(vec3 ei,vec3 ej,vec3 ek,vec3 em,vec3 eo){vec3 eu=mix(vec3(1.0),em.yyy,eo);vec3 ev=mix(vec3(1.0),em.zzz,eo);return(ei+ej*eu)+ek*ev;}vec3 dl(vec3 ef,float T){ef/=dot(vec3(1.0),abs(ef));vec2 eA=abs(ef.zx)-vec2(1.0,1.0);vec2 eB=vec2(ef.x<0.0?eA.x:-eA.x,ef.z<0.0?eA.y:-eA.y);vec2 eC=(ef.y<0.0)?eB:ef.xz;eC=vec2(0.5*(254.0/256.0),0.125*0.5*(254.0/256.0))*eC+vec2(0.5,0.125*0.5);float eD=fract(7.0*T);eC.y+=0.125*(7.0*T-eD);vec2 eE=eC+vec2(0.0,0.125);vec4 eF=mix(texture2D(tSkySpecular,eC),texture2D(tSkySpecular,eE),eD);vec3 r=eF.xyz*(7.0*eF.w);return r*r;}float dm(vec3 ef,vec3 eG){float eH=dot(ef,eG);eH=saturate(1.0+uHorizonOcclude*eH);return eH*eH;}",
		"matmicrofiber.glsl": "\n#ifdef MICROFIBER\nuniform vec4 uTexRangeFuzz;uniform float uFresnelIntegral;uniform vec4 uFresnelColor;uniform float uFresnelOcc;uniform float uFresnelGlossMask;struct dd{vec3 eI;vec3 dK;vec3 eJ;vec3 eK;vec3 eL;};void df(out dd s,vec3 L){s.eI=s.dK=di(L);s.eJ=vec3(0.0);s.eK=uFresnelColor.rgb;s.eL=uFresnelColor.aaa*vec3(1.0,0.5,0.25);\n#ifndef MICROFIBER_NO_FUZZ_TEX\nvec4 H=P(j,uTexRangeFuzz);s.eK*=J(H.rgb);\n#endif\n}void dJ(inout dd s,float eM,vec3 dE,vec3 L,vec3 dG){float dS=dot(dE,L);float dK=saturate((1.0/3.1415926)*dS);float eN=dR(dS,s.eL.z);\n#ifdef SHADOW_COUNT\ndK*=eM;float eO=mix(1.0,eM,uFresnelOcc);float eJ=eN*eO;\n#else \nfloat eJ=eN;\n#endif\ns.eJ=eJ*dG+s.eJ;s.dK=dK*dG+s.dK;}void dO(out vec3 dh,out vec3 diff_extra,inout dd s,vec3 R,vec3 L,float T){s.eJ*=uFresnelIntegral;float ed=dot(R,L);vec2 eP=ec(vec2(ed,ed),s.eL.xy);s.eJ=s.eI*eP.x+(s.eJ*eP.y);s.eJ*=s.eK;float eQ=saturate(1.0+-uFresnelGlossMask*T);s.eJ*=eQ*eQ;dh=s.dK;diff_extra=s.eJ;}\n#endif\n",
		"matsampling.glsl": "vec3 J(vec3 c){return c*c;}vec3 M(vec3 n){vec3 eR=D;vec3 eS=E;vec3 eT=gl_FrontFacing?F:-F;\n#ifdef TSPACE_RENORMALIZE\neT=normalize(eT);\n#endif\n#ifdef TSPACE_ORTHOGONALIZE\neR-=dot(eR,eT)*eT;\n#endif\n#ifdef TSPACE_RENORMALIZE\neR=normalize(eR);\n#endif\n#ifdef TSPACE_ORTHOGONALIZE\neS=(eS-dot(eS,eT)*eT)-dot(eS,eR)*eR;\n#endif\n#ifdef TSPACE_RENORMALIZE\neS=normalize(eS);\n#endif\n#ifdef TSPACE_COMPUTE_BITANGENT\nvec3 eU=cross(eT,eR);eS=dot(eU,eS)<0.0?-eU:eU;\n#endif\nn=2.0*n-vec3(1.0);return normalize(eR*n.x+eS*n.y+eT*n.z);}vec3 O(vec3 t){vec3 eT=gl_FrontFacing?F:-F;return normalize(D*t.x+E*t.y+eT*t.z);}vec4 P(vec2 eV,vec4 eW){\n#if GL_OES_standard_derivatives\nvec2 eX=fract(eV);vec2 eY=fwidth(eX);float eZ=(eY.x+eY.y)>0.5?-6.0:0.0;return texture2D(tExtras,eX*eW.xy+eW.zw,eZ);\n#else\nreturn texture2D(tExtras,fract(eV)*eW.xy+eW.zw);\n#endif\n}vec3 fc(sampler2D fd,vec2 fe,float ff){vec3 n=texture2D(fd,fe,ff*4.0).xyz;return M(n);}",
		"matshadows.glsl": "\n#ifdef SHADOW_COUNT\n#ifdef MOBILE\n#define SHADOW_KERNEL (4.0/1536.0)\n#else\n#define SHADOW_KERNEL (4.0/2048.0)\n#endif\nhighp vec4 m(highp mat4 o,highp vec3 p){return o[0]*p.x+(o[1]*p.y+(o[2]*p.z+o[3]));}uniform sampler2D tDepth0;\n#if SHADOW_COUNT > 1\nuniform sampler2D tDepth1;\n#if SHADOW_COUNT > 2\nuniform sampler2D tDepth2;\n#endif\n#endif\nuniform highp vec2 uShadowKernelRotation;uniform highp vec4 uShadowMapSize;uniform highp mat4 uShadowMatrices[SHADOW_COUNT];uniform highp vec4 uShadowTexelPadProjections[SHADOW_COUNT];highp float fh(highp vec3 B){return(B.x+B.y*(1.0/255.0))+B.z*(1.0/65025.0);}float fi(sampler2D fj,highp vec2 eV,highp float fk){\n#ifndef MOBILE\nhighp vec2 c=eV*uShadowMapSize.xy;highp vec2 a=floor(c)*uShadowMapSize.zw,b=ceil(c)*uShadowMapSize.zw;vec4 fl;fl.x=fk<fh(texture2D(fj,a).xyz)?1.0:0.0;fl.y=fk<fh(texture2D(fj,vec2(b.x,a.y)).xyz)?1.0:0.0;fl.z=fk<fh(texture2D(fj,vec2(a.x,b.y)).xyz)?1.0:0.0;fl.w=fk<fh(texture2D(fj,b).xyz)?1.0:0.0;highp vec2 w=c-a*uShadowMapSize.xy;vec2 t=(w.y*fl.zw+fl.xy)-w.y*fl.xy;return(w.x*t.y+t.x)-w.x*t.x;\n#else\nhighp float B=fh(texture2D(fj,eV.xy).xyz);return fk<B?1.0:0.0;\n#endif\n}float fm(sampler2D fj,highp vec3 eV,float fn){highp vec2 v=uShadowKernelRotation*fn;float s;s=fi(fj,eV.xy+v,eV.z);s+=fi(fj,eV.xy-v,eV.z);s+=fi(fj,eV.xy+vec2(-v.y,v.x),eV.z);s+=fi(fj,eV.xy+vec2(v.y,-v.x),eV.z);s*=0.25;return s*s;}struct dv{float dI[LIGHT_COUNT];};void dB(out dv ss,float fn){highp vec3 fo[SHADOW_COUNT];vec3 eT=gl_FrontFacing?F:-F;for(int u=0;u<SHADOW_COUNT;++u){vec4 fu=uShadowTexelPadProjections[u];float fv=fu.x*C.x+(fu.y*C.y+(fu.z*C.z+fu.w));\n#ifdef MOBILE\nfv*=.001+fn;\n#else\nfv*=.0005+0.5*fn;\n#endif\nhighp vec4 fA=m(uShadowMatrices[u],C+fv*eT);fo[u]=fA.xyz/fA.w;}\n#if SHADOW_COUNT > 0\nss.dI[0]=fm(tDepth0,fo[0],fn);\n#endif\n#if SHADOW_COUNT > 1\nss.dI[1]=fm(tDepth1,fo[1],fn);\n#endif\n#if SHADOW_COUNT > 2\nss.dI[2]=fm(tDepth2,fo[2],fn);\n#endif\nfor(int u=SHADOW_COUNT;u<LIGHT_COUNT;++u){ss.dI[u]=1.0;}}\n#endif\n",
		"matskin.glsl": "\n#ifdef SKIN\nuniform vec4 uTexRangeSubdermis;uniform vec4 uTexRangeTranslucency;uniform vec4 uTexRangeFuzz;uniform vec3 uSubdermisColor;uniform vec4 uTransColor;uniform vec4 uFresnelColor;uniform float uFresnelOcc;uniform float uFresnelGlossMask;uniform float uTransSky;uniform float uFresnelIntegral;uniform float uTransIntegral;uniform float uSkinShadowBlur;uniform float uNormalSmooth;struct Y{vec3 fB;vec3 fC,fD,fE,eJ;vec3 fF,eI,fG;vec3 fH;vec3 fI;vec3 fJ;vec3 fK;float fL;float fM;float dD;};void dc(out Y s){vec4 H;\n#ifdef SKIN_NO_SUBDERMIS_TEX\ns.fB=uSubdermisColor;s.fM=1.0;\n#else \nH=P(j,uTexRangeSubdermis);s.fB=J(H.xyz);s.fM=H.w*H.w;\n#endif\ns.fK=uTransColor.rgb;s.fL=uTransColor.a;\n#ifndef SKIN_NO_TRANSLUCENCY_TEX\nH=P(j,uTexRangeTranslucency);s.fK*=J(H.xyz);\n#endif\ns.fH=fc(tNormal,j,uNormalSmooth*s.fM);vec3 fN,fO,fP;eh(fN,fO,fP,s.fH);s.eI=s.fC=fN+fO+fP;s.fF=en(fN,fO,fP,vec3(1.0,0.6667,0.25),s.fB);vec3 fQ,fR,fS;eh(fQ,fR,fS,-s.fH);s.fG=el(fQ,fR,fS,vec3(1.0,0.4444,0.0625),s.fL);s.fG*=uTransSky;s.fD=s.fE=s.eJ=vec3(0.0);s.dD=uSkinShadowBlur*s.fM;s.fB*=0.5;s.fL*=0.5;s.fI=uFresnelColor.rgb;s.fJ=uFresnelColor.aaa*vec3(1.0,0.5,0.25);\n#ifndef SKIN_NO_FUZZ_TEX\nH=P(j,uTexRangeFuzz);s.fI*=J(H.rgb);\n#endif\n}void dH(inout Y s,float eM,float fT,vec3 dE,vec3 L,vec3 dG){float dS=dot(dE,L);float fU=dot(dE,s.fH);float dK=saturate((1.0/3.1415926)*dS);vec3 fV=dU(fU,s.fB);float fW=dR(-fU,s.fL);vec3 fE=vec3(fW*fW);\n#ifdef SHADOW_COUNT\nfloat fX=fT;vec3 fY=vec3(fT);float fZ=saturate(eM-2.0*(fT*fT));fY+=fZ*s.fB;\n#endif\nfloat eN=dR(fU,s.fJ.z);\n#ifdef SHADOW_COUNT\nvec3 eO=mix(vec3(1.0),fY,uFresnelOcc);vec3 eJ=eN*eO;\n#else\nvec3 eJ=vec3(eN);\n#endif\n#ifdef SHADOW_COUNT\nfV*=fY;fE*=fX;dK*=eM;\n#endif\ns.eJ=eJ*dG+s.eJ;s.fE=fE*dG+s.fE;s.fD=fV*dG+s.fD;s.fC=dK*dG+s.fC;}void dN(out vec3 dh,out vec3 diff_extra,inout Y s,vec3 R,vec3 L,float T){s.eJ*=uFresnelIntegral;float ed=dot(R,L);vec2 eP=ec(vec2(ed,ed),s.fJ.xy);s.eJ=s.eI*eP.x+(s.eJ*eP.y);s.eJ*=s.fI;float eQ=saturate(1.0+-uFresnelGlossMask*T);s.eJ*=eQ*eQ;s.fD=s.fD*dW(s.fB)+s.fF;s.fE=s.fE*uTransIntegral+s.fG;s.fE*=s.fK;dh=mix(s.fC,s.fD,s.fM);diff_extra=(s.eJ+s.fE)*s.fM;}\n#endif\n",
		"matstrips.glsl": "\n#ifdef STRIPVIEW\nuniform float uStrips[5];uniform vec2 uStripRes;struct V{float hc[5];float bg;};void X(out V hd,inout float T,inout vec3 S){highp vec2 eV=gl_FragCoord.xy*uStripRes-vec2(1.0,1.0);eV.x+=0.25*eV.y;hd.hc[0]=step(eV.x,uStrips[0]);hd.hc[1]=step(eV.x,uStrips[1]);hd.hc[2]=step(eV.x,uStrips[2]);hd.hc[3]=step(eV.x,uStrips[3]);hd.hc[4]=step(eV.x,uStrips[4]);hd.bg=1.0-hd.hc[4];hd.hc[4]-=hd.hc[3];hd.hc[3]-=hd.hc[2];hd.hc[2]-=hd.hc[1];hd.hc[1]-=hd.hc[0];bool he=hd.hc[4]>0.0;T=he?0.5:T;S=he?vec3(0.1):S;}vec3 dQ(V hd,vec3 L,vec3 I,vec3 S,float T,vec3 dh,vec3 dk,vec3 hf){return hd.hc[0]*(L*0.5+vec3(0.5))+hd.hc[1]*I+hd.hc[2]*S+vec3(hd.hc[3]*T)+hd.hc[4]*(vec3(0.12)+0.3*dh+dk)+hd.bg*hf;}\n#endif\n",
		"matvert.glsl": "precision highp float;uniform mat4 uModelViewProjectionMatrix;uniform mat4 uSkyMatrix;attribute vec3 vPosition;attribute vec2 vTexCoord;attribute vec2 vTangent;attribute vec2 vBitangent;attribute vec2 vNormal;\n#ifdef VERTEX_COLOR\nattribute vec4 vColor;\n#endif\nvarying highp vec3 C;varying mediump vec2 j;varying mediump vec3 D;varying mediump vec3 E;varying mediump vec3 F;\n#ifdef VERTEX_COLOR\nvarying lowp vec4 G;\n#endif\nvec3 hh(vec2 hi){bool hj=(hi.y>(32767.1/65535.0));hi.y=hj?(hi.y-(32768.0/65535.0)):hi.y;vec3 r;r.xy=(2.0*65535.0/32767.0)*hi-vec2(1.0);r.z=sqrt(clamp(1.0-dot(r.xy,r.xy),0.0,1.0));r.z=hj?-r.z:r.z;return r;}vec4 m(mat4 o,vec3 p){return o[0]*p.x+(o[1]*p.y+(o[2]*p.z+o[3]));}vec3 hk(mat4 o,vec3 hi){return o[0].xyz*hi.x+o[1].xyz*hi.y+o[2].xyz*hi.z;}void main(void){gl_Position=m(uModelViewProjectionMatrix,vPosition.xyz);j=vTexCoord;D=hk(uSkyMatrix,hh(vTangent));E=hk(uSkyMatrix,hh(vBitangent));F=hk(uSkyMatrix,hh(vNormal));C=m(uSkyMatrix,vPosition.xyz).xyz;\n#ifdef VERTEX_COLOR\nG=vColor;\n#endif\n}",
		"postfrag.glsl": "precision mediump float;uniform sampler2D tInput;\n#ifdef BLOOM\nuniform sampler2D tBloom;\n#endif\n#ifdef GRAIN\nuniform sampler2D tGrain;\n#endif\n#ifdef COLOR_LUT\nuniform sampler2D tLUT;\n#endif\nuniform vec3 uScale;uniform vec3 uBias;uniform vec3 uSaturation;uniform vec4 uSharpenKernel;uniform vec3 uSharpness;uniform vec3 uBloomColor;uniform vec4 uVignetteAspect;uniform vec4 uVignette;uniform vec4 uGrainCoord;uniform vec2 uGrainScaleBias;varying vec2 d;vec3 hl(vec3 c){vec3 hm=sqrt(c);return(hm-hm*c)+c*(0.4672*c+vec3(0.5328));}void main(void){vec3 c=texture2D(tInput,d).xyz;\n#ifdef SHARPEN\nvec3 fl=texture2D(tInput,d+uSharpenKernel.xy).xyz;fl+=texture2D(tInput,d-uSharpenKernel.xy).xyz;fl+=texture2D(tInput,d+uSharpenKernel.zw).xyz;fl+=texture2D(tInput,d-uSharpenKernel.zw).xyz;vec3 hn=uSharpness.x*c-uSharpness.y*fl;c+=clamp(hn,-uSharpness.z,uSharpness.z);\n#endif\n#ifdef BLOOM\nc+=uBloomColor*texture2D(tBloom,d).xyz;\n#endif\n#ifdef VIGNETTE\nvec2 ho=d*uVignetteAspect.xy-uVignetteAspect.zw;vec3 hi=clamp(vec3(1.0,1.0,1.0)-uVignette.xyz*dot(ho,ho),0.0,1.0);vec3 hu=hi*hi;hu*=hi;c*=mix(hi,hu,uVignette.w);\n#endif\n#ifdef SATURATION\nfloat gray=dot(c,vec3(0.3,0.59,0.11));c=mix(vec3(gray,gray,gray),c,uSaturation);\n#endif\n#ifdef CONTRAST\nc=c*uScale+uBias;\n#endif\n#ifdef GRAIN\nfloat hv=uGrainScaleBias.x*texture2D(tGrain,d*uGrainCoord.xy+uGrainCoord.zw).x+uGrainScaleBias.y;c+=c*hv;\n#endif\n#ifdef REINHARD\n{c*=1.8;float hA=dot(c,vec3(0.3333));c=clamp(c/(1.0+hA),0.0,1.0);}\n#elif defined(HEJL)\n{const highp float hB=0.22,hC=0.3,hD=.1,hE=0.2,hF=.01,hG=0.3;const highp float hH=1.25;highp vec3 dL=max(vec3(0.0),c-vec3(.004));c=(dL*((hH*hB)*dL+hH*vec3(hD*hC,hD*hC,hD*hC))+hH*vec3(hE*hF,hE*hF,hE*hF))/(dL*(hB*dL+vec3(hC,hC,hC))+vec3(hE*hG,hE*hG,hE*hG))-hH*vec3(hF/hG,hF/hG,hF/hG);}\n#endif\n#ifdef COLOR_LUT\nc=clamp(c,0.0,1.0);c=(255.0/256.0)*c+vec3(0.5/256.0);c.x=texture2D(tLUT,c.xx).x;c.y=texture2D(tLUT,c.yy).y;c.z=texture2D(tLUT,c.zz).z;c*=c;\n#endif\ngl_FragColor.xyz=hl(c);gl_FragColor.w=1.0;}",
		...
		"shadowfrag.glsl": "precision highp float;varying vec2 hI;\n#ifdef ALPHA_TEST\nvarying mediump vec2 j;uniform sampler2D tAlbedo;\n#endif\nvec3 hJ(float hi){vec4 hK=vec4(1.0,255.0,65025.0,16581375.0)*hi;hK=fract(hK);hK.xyz-=hK.yzw*(1.0/255.0);return hK.xyz;}void main(void){\n#ifdef ALPHA_TEST\nfloat k=texture2D(tAlbedo,j).a;if(k<0.5){discard;}\n#endif\ngl_FragColor.xyz=hJ((hI.x/hI.y)*0.5+0.5);gl_FragColor.w=0.0;}",
		"shadowvert.glsl": "precision highp float;attribute vec3 vPosition;attribute vec2 vTexCoord;uniform mat4 uViewProjection;varying vec2 hI;\n#ifdef ALPHA_TEST\nvarying mediump vec2 j;\n#endif\nvec4 m(mat4 o,vec3 p){return o[0]*p.x+(o[1]*p.y+(o[2]*p.z+o[3]));}void main(void){gl_Position=m(uViewProjection,vPosition);hI=gl_Position.zw;\n#ifdef ALPHA_TEST\nj=vTexCoord;\n#endif\n}",
		...
		"skySH.glsl": "precision mediump float;uniform vec4 uSkyCoefficients[9];uniform float uAlpha;varying vec3 hL;void main(void){vec3 B=normalize(hL);vec3 r=uSkyCoefficients[0].xyz;r+=uSkyCoefficients[1].xyz*B.y;r+=uSkyCoefficients[2].xyz*B.z;r+=uSkyCoefficients[3].xyz*B.x;vec3 swz=B.yyz*B.xzx;r+=uSkyCoefficients[4].xyz*swz.x;r+=uSkyCoefficients[5].xyz*swz.y;r+=uSkyCoefficients[7].xyz*swz.z;vec3 sqr=B*B;r+=uSkyCoefficients[6].xyz*(3.0*sqr.z-1.0);r+=uSkyCoefficients[8].xyz*(sqr.x-sqr.y);gl_FragColor.xyz=r;gl_FragColor.w=uAlpha;}",
		"skyvert.glsl": "precision highp float;uniform mat4 uInverseSkyMatrix;uniform mat4 uViewProjection;attribute vec3 vPosition;attribute vec2 vTexCoord;\n#if SKYMODE == 3\nvarying vec3 hL;\n#else\nvarying vec2 j;\n#endif\nvec4 m(mat4 o,vec3 p){return o[0]*p.x+(o[1]*p.y+(o[2]*p.z+o[3]));}vec4 hk(mat4 o,vec3 hi){return o[0]*hi.x+o[1]*hi.y+o[2]*hi.z;}void main(void){vec3 p=m(uInverseSkyMatrix,vPosition).xyz;gl_Position=hk(uViewProjection,p);gl_Position.z-=(1.0/65535.0)*gl_Position.w;\n#if SKYMODE == 3\nhL=vPosition;hL.xy+=1e-20*vTexCoord;\n#else\nj=vTexCoord;\n#endif\n}",
